...
namespace gr
{
  namespace lora_sdr_fmcw_0
  {
    ...
    private:
      ...
      uint8_t m_n_up_req;            ///< number of FMCW up-sweeps required to trigger a detection
      ...
      uint16_t m_preamb_len; ///< Number of FMCW triangles in the external preamble
    ...
    public:
      frame_sync0_impl(uint32_t center_freq, uint32_t bandwidth, uint8_t sf, bool impl_head, std::vector<uint16_t> sync_word, uint8_t os_factor, uint16_t preamb_len);
      ~frame_sync0_impl();
