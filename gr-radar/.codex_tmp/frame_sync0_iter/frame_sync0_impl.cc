...
namespace gr
{
    namespace lora_sdr_fmcw_0
    {
        ...
        frame_sync0::sptr
        frame_sync0::make(uint32_t center_freq, uint32_t bandwidth, uint8_t sf, bool impl_head, std::vector<uint16_t> sync_word, uint8_t os_factor, uint16_t preamble_len = 8)
        {
            return gnuradio::get_initial_sptr(new frame_sync0_impl(center_freq, bandwidth, sf, impl_head, sync_word, os_factor, preamble_len));
        }

        /*
         * The private constructor
         */
        frame_sync0_impl::frame_sync0_impl(uint32_t center_freq, uint32_t bandwidth, uint8_t sf, bool impl_head, std::vector<uint16_t> sync_word, uint8_t os_factor, uint16_t preamble_len)
        ...
        void frame_sync0_impl::rebuild_reference_chirps()
        {
            ...
            // Reacquisition after the payload can land a few raw samples before the next FMCW frame,
            // so the unlocked detector needs sample-level precision at the oversampled chirp grid.
