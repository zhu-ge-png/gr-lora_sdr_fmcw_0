...
class lora_fmcw_0_test1_no_channel(gr.top_block):
    ...
    def __init__(self):
        ...
        self.SNRdB = SNRdB = 10
        self.delay_samples = delay_samples = self._calc_delay_samples(sf, samp_rate, bw)
        self.frame_sync0_capture_samples = frame_sync0_capture_samples = 8192
        self.frame_sync0_out_path = frame_sync0_out_path = os.path.join(OUTDIR, "frame_sync0_out.dat")
        try:
            os.remove(frame_sync0_out_path)
        ...
        self.mod0_tag_dbg = blocks.tag_debug(gr.sizeof_gr_complex, "MOD0", "frame_len")
        self.mux_tag_dbg = blocks.tag_debug(gr.sizeof_gr_complex, "MUX", "frame_len")
        self.frame_sync0_head = blocks.head(gr.sizeof_gr_complex * 1, frame_sync0_capture_samples)
        self.frame_sync0_sink = blocks.file_sink(gr.sizeof_gr_complex * 1, frame_sync0_out_path, False)
        self.frame_sync0_sink.set_unbuffered(True)
        ...
        self.connect((self.blocks_tagged_stream_mux_0, 0), (self.blocks_delay_0, 0))
        ...
        self.connect((self.radar_signal_generator_fmcw_c_0, 0), (self.radar_tag_dbg, 0))
        self.connect((self.lora_sdr_fmcw_0_modulate0_0, 0), (self.mod0_tag_dbg, 0))
        self.connect((self.blocks_tagged_stream_mux_0, 0), (self.mux_tag_dbg, 0))
        self.connect((self.lora_sdr_fmcw_0_frame_sync0_0, 0), (self.frame_sync0_head, 0))
        self.connect((self.frame_sync0_head, 0), (self.frame_sync0_sink, 0))

    def _calc_delay_samples(self, sf, samp_rate, bw):
        return int((2 ** sf) * samp_rate / bw * 10.1)

    def _update_delay(self):
        self.delay_samples = self._calc_delay_samples(self.sf, self.samp_rate, self.bw)
        self.blocks_delay_0.set_dly(self.delay_samples)
...
def main(top_block_cls=lora_fmcw_0_test1_no_channel, options=None):
    tb = top_block_cls()
    ...
    try:
        size = os.path.getsize(tb.frame_sync0_out_path)
    except OSError:
        size = 0
    print(f"[debug] frame_sync0_out -> {tb.frame_sync0_out_path}")
    print(f"[debug] frame_sync0_out bytes = {size}")
