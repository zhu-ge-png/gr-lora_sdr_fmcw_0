...
namespace gr
{
    namespace lora_sdr_fmcw_0
    {
        ...
        modulate0::sptr
        modulate0::make(uint8_t sf, uint32_t samp_rate, uint32_t bw, std::vector<uint16_t> sync_words, uint32_t frame_zero_padd, uint16_t preamble_len, uint16_t zero_preamble_chirps)
        {
            return gnuradio::get_initial_sptr(new modulate0_impl(sf, samp_rate, bw, sync_words, frame_zero_padd, preamble_len, zero_preamble_chirps));
        }
        /*
     * The private constructor
     */
        modulate0_impl::modulate0_impl(uint8_t sf, uint32_t samp_rate, uint32_t bw, std::vector<uint16_t> sync_words, uint32_t frame_zero_padd, uint16_t preamble_len, uint16_t zero_preamble_chirps)
            : gr::block("modulate0",
                        ...
                        gr::io_signature::make(1, 1, sizeof(gr_complex)))
        {
            ...
            m_zero_preamble_chirps = std::min<uint16_t>(zero_preamble_chirps, m_preamb_len);
            samp_cnt = -1;
            preamb_samp_cnt = 0;
            frame_cnt = 0;
     ...
     */
        modulate0_impl::~modulate0_impl()
        {
        }

        void
        modulate0_impl::forecast(int noutput_items, gr_vector_int &ninput_items_required)
        {
            ninput_items_required[0] = m_ninput_items_required;
        }

        void modulate0_impl::update_var(int new_sf, int new_bw)
        {
            if (new_sf != m_sf) {
                m_sf = new_sf;
        ...
        int modulate0_impl::general_work(int noutput_items,
                                        ...
                                        gr_vector_void_star &output_items)
        {
            ...
            if (tags.size())
            {
                ...
                else
                {
                    ...
                    if (frame_end)
                    {
                        ...
                        m_framelen_tag.value = pmt::from_long(int((m_frame_len + m_zero_preamble_chirps + 4.25) * m_samples_per_symbol + m_inter_frame_padding ));
            ...
            if (samp_cnt == -1) // external FMCW preamble is provided by the radar branch; output retained 0-bin chirps plus LoRa sync tail
            {
                const int32_t zero_chirp_samples = m_zero_preamble_chirps * m_samples_per_symbol;
                const int32_t sync_tail_start = zero_chirp_samples;
                const int32_t sync_tail_end = sync_tail_start + 5 * m_samples_per_symbol;
                for (int i = 0; i < noutput_items / m_samples_per_symbol; i++)
                {
                    if (preamb_samp_cnt < sync_tail_end) // retained 0-bin chirps + sync1 + sync2 + 2.25 downchirps
                    {
                        const int32_t sync_tail_samp_cnt = preamb_samp_cnt - sync_tail_start;
                        if (preamb_samp_cnt < zero_chirp_samples) // retained 0-bin upchirps
                            memcpy(&out[output_offset], &m_upchirp[0], m_samples_per_symbol * sizeof(gr_complex));
                        else if (sync_tail_samp_cnt == 0) // sync word 1
                            build_upchirp(&out[output_offset], m_sync_words[0], m_sf,m_os_factor);
                        else if (sync_tail_samp_cnt == 1 * m_samples_per_symbol) // sync word 2
                            build_upchirp(&out[output_offset], m_sync_words[1], m_sf,m_os_factor);
                        else if (sync_tail_samp_cnt < 4 * m_samples_per_symbol) // 2 downchirps
                            memcpy(&out[output_offset], &m_downchirp[0], m_samples_per_symbol * sizeof(gr_complex));
                        else if (sync_tail_samp_cnt == 4 * m_samples_per_symbol) // final quarter downchirp
                        {
