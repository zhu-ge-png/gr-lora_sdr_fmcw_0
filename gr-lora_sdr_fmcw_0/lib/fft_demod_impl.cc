...
namespace gr {
    namespace lora_sdr_fmcw_0 {
        ...
        fft_demod_impl::fft_demod_impl(bool soft_decoding, bool max_log_approx)
            ...
            energy_file.open("../../matlab/measurements/energy" + std::to_string(num) + ".txt",
...
#ifdef GRLORA_MEASUREMENTS
            energy_file << std::fixed << std::setprecision(10) << m_fft_mag_sq[idx] << ","
                        << m_fft_mag_sq[mod(idx - 1, m_samples_per_symbol)] << ","
                        << m_fft_mag_sq[mod(idx + 1, m_samples_per_symbol)] << "," << rec_en << ","
                        << std::endl;
#endif
        ...
        std::vector<LLR> fft_demod_impl::get_LLRs(const gr_complex* samples) {
            ...
            for (uint32_t n = 0; n < m_samples_per_symbol; n++) {
                bessel_file << std::setprecision(8) << std::sqrt(Ps_frame) / Pn_frame * std::sqrt(m_fft_mag_sq[n])
                            << "," << Ps_frame << "," << Pn_frame << "," << m_fft_mag_sq[n] << std::endl;
            }
#endif
            ...
            for (uint32_t n = 0; n < m_samples_per_symbol; n++) {
                ...
                if (max_log_approx)
                    LLs[n] = std::log(LLs[n]);  // Log-Likelihood
            ...
            LL_file.close();
            for (uint32_t i = 0; i < m_sf; i++)
                LLR_file << std::fixed << std::setprecision(10) << LLRs[i] << std::endl;
            LLR_file.close();
#endif
