...
namespace gr
{
    namespace lora_sdr_fmcw_0
    {
        namespace
        {
            ...
            constexpr float kDetectFftDownConfirmThreshold = 0.10f;
            constexpr float kDetectFftDownConfirmMargin = 0.03f;
            constexpr float kDetectTrackingCorrThreshold = 0.25f;
            constexpr int kDetectTrackingStartCount = 2;
            constexpr std::array<float, 9> kDetectCfoBinHypotheses = {
                -2.0f, -1.5f, -1.0f, -0.5f, 0.0f, 0.5f, 1.0f, 1.5f, 2.0f};

            float template_energy(const std::vector<gr_complex>& samples)
            {
                float energy = 0.0f;
            ...
            float signal_energy(const gr_complex* in, size_t len)
            {
                float energy = 0.0f;
                for (size_t i = 0; i < len; ++i)
                    energy += std::norm(in[i]);
                return std::max(energy, 1.0e-12f);
            }

            float normalized_corr(const gr_complex* in,
                                  ...
                                  float tmpl_energy)
            {
                ...
                {
                    dot += in[i] * std::conj(tmpl[i]);
                    sig_energy += std::norm(in[i]);
                }

                const float denom = std::sqrt(std::max(sig_energy * tmpl_energy, 1.0e-20f));
                return std::abs(dot) / denom;
            }

            float normalized_corr_cfo_bank(const gr_complex* in,
                                           const std::vector<gr_complex>& tmpl,
                                           float tmpl_energy,
                                           float sig_energy)
            {
                ...
                for (const float delta_bins : kDetectCfoBinHypotheses)
                {
                    const gr_complex phase_step =
                        gr_expj(2 * M_PI * delta_bins / static_cast<float>(tmpl.size()));
                    gr_complex phase(1.0f, 0.0f);
                    gr_complex dot(0.0f, 0.0f);

                    for (size_t i = 0; i < tmpl.size(); ++i)
                    {
                        dot += in[i] * std::conj(tmpl[i] * phase);
                        phase *= phase_step;
                    }

                    best_corr = std::max(best_corr, std::abs(dot) / denom);
                }

                return best_corr;
            }

            DetectWindowKind classify_detect_window(const gr_complex* in,
                                                    const std::vector<gr_complex>& upchirp,
                                                    const std::vector<gr_complex>& downchirp,
                                                    float up_energy,
                                                    float down_energy,
                                                    float corr_threshold,
                                                    float corr_margin,
                                                    bool allow_cfo_bank,
                                                    float* up_corr_out = nullptr,
                                                    float* down_corr_out = nullptr)
            {
                float up_corr = normalized_corr(in, upchirp, up_energy);
                float down_corr = normalized_corr(in, downchirp, down_energy);

                if (allow_cfo_bank && (up_corr < corr_threshold || down_corr < corr_threshold))
                {
                    const float sig_energy = signal_energy(in, upchirp.size());
                    if (up_corr < corr_threshold)
                        up_corr = normalized_corr_cfo_bank(in, upchirp, up_energy, sig_energy);
                    if (down_corr < corr_threshold)
                        down_corr = normalized_corr_cfo_bank(in, downchirp, down_energy, sig_energy);
                }

                if (up_corr_out != nullptr)
                    *up_corr_out = up_corr;
                if (down_corr_out != nullptr)
                    *down_corr_out = down_corr;

                if (up_corr >= corr_threshold && up_corr > down_corr + corr_margin)
                    return DETECT_UP;
                if (down_corr >= corr_threshold && down_corr > up_corr + corr_margin)
                    return DETECT_DOWN;
                return DETECT_UNKNOWN;
            }

            int down_bin_to_cfo_int(int down_bin, uint32_t number_of_bins)
            {
                const int signed_down_bin =
                    (down_bin < (int)(number_of_bins / 2)) ? down_bin : (down_bin - (int)number_of_bins);
                return (signed_down_bin >= 0) ? (signed_down_bin / 2) : (int)std::floor(signed_down_bin / 2.0);
            }
            ...
            std::string format_valid_int_vector(const std::vector<int>& values, const std::vector<uint8_t>& valid)
            {
                std::ostringstream oss;
                oss << "[";
                bool first = true;
                for (size_t ii = 0; ii < values.size() && ii < valid.size(); ++ii)
                {
                    if (!valid[ii])
                        continue;
                    if (!first)
        ...
        frame_sync0::sptr
        frame_sync0::make(double center_freq, uint32_t bandwidth, uint8_t sf, bool impl_head, std::vector<uint16_t> sync_word, uint8_t os_factor, uint16_t preamble_len, float detect_corr_threshold, int fmcw_preamble_len)
        {
            return gnuradio::get_initial_sptr(new frame_sync0_impl(center_freq, bandwidth, sf, impl_head, sync_word, os_factor, preamble_len, detect_corr_threshold, fmcw_preamble_len));
        }

        /*
         * The private constructor
         */
        frame_sync0_impl::frame_sync0_impl(double center_freq, uint32_t bandwidth, uint8_t sf, bool impl_head, std::vector<uint16_t> sync_word, uint8_t os_factor, uint16_t preamble_len, float detect_corr_threshold, int fmcw_preamble_len)
            : gr::block("frame_sync0",
                        ...
                        gr::io_signature::make2(1, 2, sizeof(gr_complex), sizeof(float)))
        {
            ...
            m_bw = bandwidth;
            m_samp_rate = bandwidth * os_factor;
            m_sf = sf;
            ...
            m_fmcw_preamb_len =
                static_cast<uint16_t>(std::min<int>(std::max<int>(0, fmcw_preamble_len < 0 ? preamble_len : fmcw_preamble_len), preamble_len));
            net_ids.resize(2, 0);

            m_triangle_stride_sym = 2;
            m_sync_handoff_stride_sym = (m_fmcw_preamb_len >= m_preamb_len) ? m_triangle_stride_sym : 1;
            m_n_up_req = preamble_len;
            up_symb_to_use = std::max<int>(1, m_n_up_req - 1);

            m_sto_frac = 0.0f;
            m_cfo_frac = 0.0f;
            m_cfo_frac_bernier = 0.0f;
            m_cfo_int = 0;
            sfo_hat = 0.0f;
            sfo_cum = 0.0f;
            cfo_frac_sto_frac_est = false;
            m_detect_up_energy = 0.0f;
            m_detect_down_energy = 0.0f;
            m_detect_corr_threshold = std::clamp(detect_corr_threshold, 0.0f, 1.0f);
            m_detect_corr_margin = 0.05f;
            m_detect_search_step = 1;
            ...
            preamble_upchirps.resize(m_n_up_req * m_number_of_bins);
            preamble_raw_up.resize((m_n_up_req + 3) * m_samples_per_symbol);
            preamb_down_vals.resize(m_n_up_req, 0);
            preamb_down_valid.resize(m_n_up_req, 0);
            ...
            m_detect_cfo_int_hint = 0;
            m_detect_cfo_int_hint_valid = false;
            frame_cnt = 0;
            m_symb_numb = 0;
            one_symbol_off = 0;
            down_val = 0;
         ...
         */
        frame_sync0_impl::~frame_sync0_impl()
        {
            delete[] cx_out;
            delete[] cx_in;
        ...
        void frame_sync0_impl::set_detect_corr_threshold(float detect_corr_threshold)
        {
            m_detect_corr_threshold = std::clamp(detect_corr_threshold, 0.0f, 1.0f);
        }

        int frame_sync0_impl::my_roundf(float number)
        {
            int ret_val = (int)(number > 0 ? int(number + 0.5) : std::ceil(number - 0.5));
            return ret_val;
        }

        int frame_sync0_impl::preamble_up_spacing_after(int up_index) const
        {
            return (up_index < static_cast<int>(m_fmcw_preamb_len)) ? m_triangle_stride_sym : 1;
        }

        float frame_sync0_impl::preamble_to_sync_span_symbols() const
        {
            return static_cast<float>(m_preamb_len + m_fmcw_preamb_len);
        }

        void frame_sync0_impl::rebuild_reference_chirps()
        {
            m_upchirp.resize(m_number_of_bins);
            m_downchirp.resize(m_number_of_bins);
            m_detect_upchirp.resize(m_samples_per_symbol);
            m_detect_downchirp.resize(m_samples_per_symbol);

            build_ref_chirps(&m_upchirp[0], &m_downchirp[0], m_sf);
            build_ref_chirps(&m_detect_upchirp[0], &m_detect_downchirp[0], m_sf, m_os_factor);

            m_detect_up_energy = template_energy(m_detect_upchirp);
            m_detect_down_energy = template_energy(m_detect_downchirp);
            m_detect_search_step = std::max<int>(1, (int)m_os_factor);
        }

        void frame_sync0_impl::reset_detect_history()
        {
            std::fill(preamb_down_vals.begin(), preamb_down_vals.end(), 0);
            std::fill(preamb_down_valid.begin(), preamb_down_valid.end(), 0);
            m_detect_cfo_int_hint = 0;
            m_detect_cfo_int_hint_valid = false;
        }
        void frame_sync0_impl::forecast(int noutput_items, gr_vector_int &ninput_items_required)
        {
            ninput_items_required[0] = (m_os_factor * (m_triangle_stride_sym * m_number_of_bins + 2));
        }

        float frame_sync0_impl::estimate_CFO_frac(gr_complex *samples)
        {
            int k0;
            float cfo_frac;
        ...
        float frame_sync0_impl::estimate_CFO_frac_Bernier(gr_complex *samples)
        {
            ...
            double phase_num = 0.0;
            double phase_den = 0.0;
            for (int i = 0; i < up_symb_to_use - 1; i++)
            {
                const gr_complex adjacent_phase =
                    fft_val[idx_max + m_number_of_bins * i] *
                    std::conj(fft_val[idx_max + m_number_of_bins * (i + 1)]);
                const int up_spacing = preamble_up_spacing_after(i);
                phase_num += up_spacing * std::arg(adjacent_phase);
                phase_den += up_spacing * up_spacing;
            }
            cfo_frac = (phase_den > 0.0) ? -phase_num / (2 * M_PI * phase_den) : 0.0f;
        ...
        float frame_sync0_impl::estimate_STO_frac()
        {
            int k0;
            double Y_1, Y0, Y1, u, v, ka, wa, k_residual;
            float sto_frac = 0;
        ...
        uint32_t frame_sync0_impl::get_symbol_val(const gr_complex *samples, gr_complex *ref_chirp)
        {
            int32_t bin_idx_local = -1;
            get_symbol_peak_share(samples, ref_chirp, &bin_idx_local);
            return bin_idx_local;
        }

        float frame_sync0_impl::get_symbol_peak_share(const gr_complex *samples, gr_complex *ref_chirp, int32_t *bin_out)
        {
            double sig_en = 0;
            ...
            const auto max_it = std::max_element(std::begin(fft_mag), std::end(fft_mag));
            if (bin_out != nullptr)
                *bin_out = std::distance(std::begin(fft_mag), max_it);

            return *max_it / sig_en;
        }

        float frame_sync0_impl::determine_energy(const gr_complex *samples, int length = 1)
        {
        ...
        float frame_sync0_impl::determine_snr(const gr_complex *samples)
        {
            double tot_en = 0;
            std::vector<float> fft_mag(m_number_of_bins);
            std::vector<gr_complex> dechirped(m_number_of_bins);
        ...
        void frame_sync0_impl::noise_est_handler(pmt::pmt_t noise_est)
        {
            m_noise_est = pmt::to_double(noise_est);
        }
        void frame_sync0_impl::frame_info_handler(pmt::pmt_t frame_info)
        {
        ...
        void frame_sync0_impl::set_sf(int sf)
        {
            m_sf = sf;
            m_number_of_bins = (uint32_t)(1u << m_sf);
            m_samp_rate = m_bw * m_os_factor;
            m_samples_per_symbol = m_number_of_bins * m_os_factor;
            additional_symbol_samp.resize(2 * m_samples_per_symbol);
            preamble_upchirps.resize(m_n_up_req * m_number_of_bins);
            preamble_raw_up.resize((m_n_up_req + 3) * m_samples_per_symbol);
        ...
        int frame_sync0_impl::general_work(int noutput_items,
                                          ...
                                          gr_vector_void_star &output_items)
        {
            ...
            case DETECT:
            {
                const int search_step = std::max(1, m_detect_search_step);
                const float active_corr_threshold =
                    (symbol_cnt >= kDetectTrackingStartCount)
                        ? std::min(m_detect_corr_threshold, kDetectTrackingCorrThreshold)
                        : m_detect_corr_threshold;
                float up_corr = 0.0f;
                float down_corr = 0.0f;
                DetectWindowKind detect_kind = classify_detect_window(
                    in,
                    m_detect_upchirp,
                    m_detect_downchirp,
                    m_detect_up_energy,
                    m_detect_down_energy,
                    active_corr_threshold,
                    m_detect_corr_margin,
                    symbol_cnt > 0,
                    &up_corr,
                    &down_corr);

                if (detect_kind == DETECT_UP)
                {
                    float up_peak_share = 0.0f;
                    float down_peak_share = 0.0f;
                    int32_t up_bin_idx = -1;

                    up_peak_share = get_symbol_peak_share(&in_down[0], &m_downchirp[0], &up_bin_idx);
                    down_peak_share = get_symbol_peak_share(&in_down[0], &m_upchirp[0], nullptr);

                    if (up_bin_idx == -1)
                    {
                        detect_kind = DETECT_UNKNOWN;
                    }
                    else if (down_peak_share >= kDetectFftDownConfirmThreshold &&
                             down_peak_share > up_peak_share + kDetectFftDownConfirmMargin)
                    {
                        detect_kind = DETECT_DOWN;
                    }
                    else
                    {
                        bin_idx_new = up_bin_idx;
                    }
                }

                if (detect_kind == DETECT_DOWN)
                {
                    if (symbol_cnt == 0)
                    {
                        bin_idx = -1;
                        bin_idx_new = -1;
                        items_to_consume = search_step;
                    }
                    else
                    {
                        int32_t down_bin_idx = -1;
                        get_symbol_peak_share(&in_down[0], &m_upchirp[0], &down_bin_idx);
                        if (down_bin_idx != -1)
                        {
                            const int down_store_idx =
                                std::min<int>(symbol_cnt - 1, (int)preamb_down_vals.size() - 1);
                            preamb_down_vals[down_store_idx] = down_bin_idx;
                            preamb_down_valid[down_store_idx] = 1;
                        }
                ...
                const bool bin_consistent =
                    (symbol_cnt == 0) ||
                    (bin_idx != -1 && abs(mod(abs(bin_idx_new - bin_idx) + 1, m_number_of_bins) - 1) <= 1);
                const int store_idx = bin_consistent ? symbol_cnt : 0;

                if (store_idx == 0)
                    reset_detect_history();

                preamb_up_vals[store_idx] = bin_idx_new;
                memcpy(&preamble_raw[m_number_of_bins * store_idx], &in_down[0], m_number_of_bins * sizeof(gr_complex));
                memcpy(&preamble_raw_up[m_samples_per_symbol * store_idx], &in[(int)(m_os_factor / 2)], m_samples_per_symbol * sizeof(gr_complex));

                symbol_cnt = store_idx + 1;
                bin_idx = bin_idx_new;
                if (symbol_cnt == (int)(m_n_up_req))
                {
                    ...
                    symbol_cnt = NET_ID1;
                    cfo_frac_sto_frac_est = false;
                    k_hat = most_frequent(&preamb_up_vals[0], m_n_up_req);
                    std::vector<int> detect_cfo_candidates;
                    detect_cfo_candidates.reserve(preamb_down_vals.size());
                    for (size_t ii = 0; ii < preamb_down_vals.size(); ++ii)
                    {
                        if (preamb_down_valid[ii])
                            detect_cfo_candidates.push_back(
                                down_bin_to_cfo_int(preamb_down_vals[ii], m_number_of_bins));
                    }
                    if (detect_cfo_candidates.size() >= 2)
                    {
                        m_detect_cfo_int_hint =
                            most_frequent(&detect_cfo_candidates[0], detect_cfo_candidates.size());
                        m_detect_cfo_int_hint_valid = true;
                    }
                    std::cout << "[frame_sync0 detect_lock] in=" << nitems_read(0)
                              << " k_hat=" << k_hat
                              << " down_bins=" << format_valid_int_vector(preamb_down_vals, preamb_down_valid)
                              << " down_cfo_candidates=" << format_int_vector(detect_cfo_candidates)
                              << " down_cfo_hint="
                              << (m_detect_cfo_int_hint_valid ? std::to_string(m_detect_cfo_int_hint) : std::string("none"))
                              << std::endl;
                    memcpy(&net_id_samp[0],
                           &in[int((m_sync_handoff_stride_sym - 0.25f) * m_samples_per_symbol - k_hat * m_os_factor)],
                           sizeof(gr_complex) * 0.25 * m_samples_per_symbol);

                    items_to_consume = m_os_factor * ((int)(m_sync_handoff_stride_sym * m_number_of_bins - k_hat));
                }
                else
                    items_to_consume = m_samples_per_symbol;
                items_to_output = 0;
            ...
            case SYNC:
            {
                ...
                case QUARTER_DOWN:
                {
                    ...
                    const int sync_down_cfo_int = down_bin_to_cfo_int(down_val, m_number_of_bins);
                    m_cfo_int = sync_down_cfo_int;
                    if (m_detect_cfo_int_hint_valid &&
                        std::abs(m_detect_cfo_int_hint - sync_down_cfo_int) <= 1)
                    {
                        m_cfo_int = m_detect_cfo_int_hint;
                    }
                    std::cout << "[frame_sync0 sync_down] in=" << nitems_read(0)
                              << " down_bin=" << down_val
                              << " sync_down_cfo_int=" << sync_down_cfo_int
                              << " detect_cfo_hint="
                              << (m_detect_cfo_int_hint_valid ? std::to_string(m_detect_cfo_int_hint) : std::string("none"))
                              << " selected_cfo_int=" << m_cfo_int
                              << std::endl;
                    ...
                    CFO_int_correc.resize(m_n_up_req * m_number_of_bins);
                    for (uint32_t n = 0; n < m_n_up_req * m_number_of_bins; n++)
                    {
                    ...
                    sfo_corr_vect.resize(m_n_up_req * m_number_of_bins, 0);
                    for (uint32_t n = 0; n < m_n_up_req * m_number_of_bins; n++)
                    {
                    ...
                    corr_preamb.resize(m_n_up_req * m_number_of_bins, 0);
                    // apply sto correction
                    for (uint32_t i = 0; i < m_n_up_req * m_number_of_bins; i++)
                    {
                    ...
                    // apply cfo correction
                    volk_32fc_x2_multiply_32fc(&corr_preamb[0], &corr_preamb[0], &CFO_int_correc[0], m_n_up_req * m_number_of_bins);
                    for (int i = 0; i < m_n_up_req; i++)
                    {
                    ...
                    volk_32fc_x2_multiply_32fc(&corr_preamb[0], &corr_preamb[0], &sfo_corr_vect[0], m_n_up_req * m_number_of_bins);
                    ...
                    m_sto_frac += sfo_hat * preamble_to_sync_span_symbols();
                    ...
                    std::cout << "[frame_sync0 sync_ids] in=" << nitems_read(0)
                              << " netid1=" << netid1
                              << " netid2=" << netid2
                              << " expect1=" << (int)m_sync_words[0]
                              << " expect2=" << (int)m_sync_words[1]
                              << std::endl;
                    one_symbol_off = 0;
                    ...
                    else if (abs(netid1 - (int32_t)m_sync_words[0]) > 2) // wrong id 1, (we allow an offset of 2)
                    {
                        ...
                        if (abs(netid1 - (int32_t)m_sync_words[1]) <= 2)
                        {
                            ...
                            for (int i = m_n_up_req - 2; i < m_n_up_req; i++)
                            {
                                if (get_symbol_val(&corr_preamb[i * m_number_of_bins], &m_downchirp[0]) + net_id_off == m_sync_words[0]) // found the first netID
                                {
                                    ...
                                    if (net_id_off != 0 && abs(net_id_off) > 1)
                                        std::cout << RED << "[frame_sync0_impl.cc] net id offset >1: " << net_id_off << RESET << std::endl;
                                    if (m_should_log)
                    ...
                    if (m_state != DETECT)
                    {
                        ...
                        std::cout << "[frame_sync0_impl.cc] " << frame_cnt << " CFO estimate: " << m_cfo_int + m_cfo_frac << ", STO estimate: " << k_hat - m_cfo_int + m_sto_frac << " snr est: " << snr_est << std::endl;
#endif
                    }
                }
                }
            ...
            case SFO_COMPENSATION:
            {
                ...
                {
                    std::cout << "[frame_sync0 frame_done] in=" << nitems_read(0)
                              << " out=" << nitems_written(0)
                              << std::endl;
                    m_state = DETECT;
                    symbol_cnt = 0;
                    items_to_consume = 0;
