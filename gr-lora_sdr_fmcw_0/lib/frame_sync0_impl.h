...
#ifndef INCLUDED_LORA_SDR_FMCW_0_FRAME_SYNC0_IMPL_H
#define INCLUDED_LORA_SDR_FMCW_0_FRAME_SYNC0_IMPL_H
// #define GRLORA_DEBUG
// #define PRINT_INFO
...
namespace gr
{
  namespace lora_sdr_fmcw_0
  {
    ...
    class frame_sync0_impl : public frame_sync0
    {
    private:
      enum DecoderState
      {
      ...
      double m_center_freq;               ///< RF center frequency
      ...
      uint8_t m_n_up_req;            ///< total number of preamble upchirps required to trigger a detection
      ...
      uint8_t m_triangle_stride_sym; ///< number of LoRa-length symbols per FMCW triangle
      uint8_t m_sync_handoff_stride_sym; ///< symbols from the final detected upchirp to the first sync word
      ...
      std::vector<gr_complex> m_detect_upchirp;   ///< oversampled 0-bin upchirp template for FMCW up-sweeps
      std::vector<gr_complex> m_detect_downchirp; ///< oversampled downchirp template used to reject FMCW down-sweeps
      float m_detect_up_energy;             ///< energy of the oversampled upchirp template
      float m_detect_down_energy;           ///< energy of the oversampled downchirp template
      float m_detect_corr_threshold;        ///< minimum normalized correlation used to accept a window
      float m_detect_corr_margin;           ///< minimum gap between up/down correlations when classifying a window
      int m_detect_search_step;            ///< search stride used before the detector locks to a triangle grid
      ...
      uint16_t m_fmcw_preamb_len; ///< number of external FMCW triangles in the preamble
      uint8_t additional_upchirps; ///< extra preamble-like symbols absorbed while refining sync handoff
      ...
      std::vector<int> preamb_down_vals;        ///< value of the FMCW down-sweeps captured during DETECT
      std::vector<uint8_t> preamb_down_valid;   ///< indicate which FMCW down-sweeps were decoded reliably
      ...
      int m_detect_cfo_int_hint;       ///< integer CFO hint derived from FMCW down-sweeps
      bool m_detect_cfo_int_hint_valid; ///< indicate that the preamble down-based CFO hint is valid
      ...
      float down_val_frac;               ///< fractional-bin value of the preamble downchirps
      ...
      float get_symbol_peak_share(const gr_complex *samples, gr_complex *ref_chirp, int32_t *bin_out = nullptr);
      ...
      void rebuild_reference_chirps();
      void reset_detect_history();
      int preamble_up_spacing_after(int up_index) const;
      float preamble_to_sync_span_symbols() const;

    public:
      frame_sync0_impl(double center_freq, uint32_t bandwidth, uint8_t sf, bool impl_head, std::vector<uint16_t> sync_word, uint8_t os_factor, uint16_t preamb_len, float detect_corr_threshold = 0.45f, int fmcw_preamble_len = -1);
      ~frame_sync0_impl();
      void set_detect_corr_threshold(float detect_corr_threshold) override;
...
#endif /* INCLUDED_LORA_SDR_FMCW_0_FRAME_SYNC0_IMPL_H */
