...
namespace gr
{
     ...
     */
        ...
        void header_decoder_impl::publish_frame_info(int cr, int pay_len, int crc, uint8_t ldro_mode, int err)
        {
            ...
            std::cout << "[header_decoder frame_info] cr=" << cr
                      << " pay_len=" << pay_len
                      << " crc=" << crc
                                              ...
                                              gr_vector_void_star &output_items)
        {
            ...
            if (is_header)
            {
                ...
                { //explicit header to decode
                    ...
                    std::cout << "[header_decoder hdrsym] in0=" << int(in[0])
                              << " in1=" << int(in[1])
                              << " in2=" << int(in[2])
                              << " in3=" << int(in[3])
                              << " in4=" << int(in[4])
                    ...
                    std::cout << "[header_decoder chk] payload_len=" << int(m_payload_len)
                              << " has_crc=" << int(m_has_crc)
                              << " cr=" << int(m_cr)
                              << " header_chk=" << int(header_chk)
                              << " computed_chk=" << int((c4 << 4) + (c3 << 3) + (c2 << 2) + (c1 << 1) + c0)
                              << " head_err=" << head_err
                              << std::endl;
                    ...
                    std::cout << "[header_decoder payload_fwd] head_err=" << head_err
                              << " nitem_to_process=" << nitem_to_process
