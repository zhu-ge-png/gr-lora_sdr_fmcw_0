...
namespace gr
{
    namespace lora_sdr_fmcw_0
    {
        ...
        frame_sync0::sptr
        frame_sync0::make(double center_freq, uint32_t bandwidth, uint8_t sf, bool impl_head, std::vector<uint16_t> sync_word, uint8_t os_factor, uint16_t preamble_len, float detect_corr_threshold)
        {
            return gnuradio::get_initial_sptr(new frame_sync0_impl(center_freq, bandwidth, sf, impl_head, sync_word, os_factor, preamble_len, detect_corr_threshold));
        }

        /*
         * The private constructor
         */
        frame_sync0_impl::frame_sync0_impl(double center_freq, uint32_t bandwidth, uint8_t sf, bool impl_head, std::vector<uint16_t> sync_word, uint8_t os_factor, uint16_t preamble_len, float detect_corr_threshold)
        ...
        float frame_sync0_impl::estimate_CFO_frac_Bernier(gr_complex *samples)
        {
            ...
            // Consecutive FMCW training ups are separated by one down-sweep.
            cfo_frac = -std::arg(four_cum) / (2 * M_PI * m_triangle_stride_sym);
                                          ...
                                          gr_vector_void_star &output_items)
        {
            ...
            case DETECT:
            {
                ...
                if (symbol_cnt == (int)(m_n_up_req))
                {
                    ...
                    memcpy(&net_id_samp[0],
                           &in[int((m_triangle_stride_sym - 0.25f) * m_samples_per_symbol - k_hat * m_os_factor)],
                           sizeof(gr_complex) * 0.25 * m_samples_per_symbol);

                    // Skip the down half of the last FMCW triangle and land on the first sync word.
                    items_to_consume = m_os_factor * ((int)(m_triangle_stride_sym * m_number_of_bins - k_hat));
                }
                else
                    items_to_consume = m_samples_per_symbol;
                items_to_output = 0;
            ...
            case SYNC:
            {
                ...
                case QUARTER_DOWN:
                {
                    ...
                    m_sto_frac += sfo_hat * (m_triangle_stride_sym * m_preamb_len);
