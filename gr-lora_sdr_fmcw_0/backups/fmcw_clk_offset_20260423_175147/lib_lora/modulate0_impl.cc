...
namespace gr
{
    namespace lora_sdr_fmcw_0
    {
        ...
        modulate0::sptr
        modulate0::make(uint8_t sf, uint32_t samp_rate, uint32_t bw, std::vector<uint16_t> sync_words, uint32_t frame_zero_padd, uint16_t preamble_len = 8)
        {
            return gnuradio::get_initial_sptr(new modulate0_impl(sf, samp_rate, bw, sync_words, frame_zero_padd, preamble_len));
        }
        /*
     * The private constructor
     */
        modulate0_impl::modulate0_impl(uint8_t sf, uint32_t samp_rate, uint32_t bw, std::vector<uint16_t> sync_words, uint32_t frame_zero_padd, uint16_t preamble_len)
            : gr::block("modulate0",
     ...
     */
                                        ...
                                        gr_vector_void_star &output_items)
        {
            ...
            if (tags.size())
            {
                ...
                else
                {
                    ...
                    if (frame_end)
                    {
                        ...
                        m_framelen_tag.value = pmt::from_long(int((m_frame_len + 4.25) * m_samples_per_symbol + m_inter_frame_padding ));
            ...
            if (samp_cnt == -1) // external FMCW preamble is provided by the radar branch; output only LoRa sync tail
            {

                for (int i = 0; i < noutput_items / m_samples_per_symbol; i++)
                {
                    if (preamb_samp_cnt < 5 * m_samples_per_symbol) // sync1 + sync2 + 2.25 downchirps
                    {
                        if (preamb_samp_cnt == 0) // sync word 1
                            build_upchirp(&out[output_offset], m_sync_words[0], m_sf,m_os_factor);
                        else if (preamb_samp_cnt == 1 * m_samples_per_symbol) // sync word 2
                            build_upchirp(&out[output_offset], m_sync_words[1], m_sf,m_os_factor);
                        else if (preamb_samp_cnt < 4 * m_samples_per_symbol) // 2 downchirps
                            memcpy(&out[output_offset], &m_downchirp[0], m_samples_per_symbol * sizeof(gr_complex));
                        else if (preamb_samp_cnt == 4 * m_samples_per_symbol) // final quarter downchirp
                        {
