...
namespace gr
{
  namespace lora_sdr_fmcw_0
  {
    ...
    private:
      ...
      uint8_t m_n_up_req;            ///< number of consecutive FMCW triangles used to trigger detection (tri_preamble_len - 3)
      ...
      uint16_t m_preamb_len; ///< Number of FMCW triangles in external preamble
      uint8_t additional_upchirps; ///< for FMCW mode we pre-fill the three triangles skipped after detection
      ...
      std::vector<gr_complex> preamble_raw;      ///< concatenated FMCW up-sweeps (one LoRa symbol each) before synchronization
      std::vector<gr_complex> preamble_raw_up;  ///< oversampled concatenated FMCW up-sweeps before synchronization
      ...
      int down_val;                      ///< value of the LoRa downchirp tail used for integer CFO
    ...
    public:
      frame_sync0_impl(double center_freq, uint32_t bandwidth, uint8_t sf, bool impl_head, std::vector<uint16_t> sync_word, uint8_t os_factor, uint16_t preamb_len, float detect_corr_threshold = 0.45f);
      ~frame_sync0_impl();
